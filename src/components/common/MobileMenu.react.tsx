  ...
  return (
    <div className="md:hidden">
      ...
      <div
        ...
        className={`fixed top-0 right-0 z-50 w-full max-w-xs sm:max-w-sm h-full bg-white shadow-2xl transition-transform duration-300 ease-in-out flex flex-col ${
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 border-b border-slate-200">
          <div className="flex items-center gap-2">
            <img src={brandConfig.logoPath} alt={brandConfig.brandName} className="h-7 w-auto" />
          </div>
          <button
            type="button"
            onClick={() => setIsOpen(false)}
        ...
        <nav className="flex-1 overflow-y-auto px-5 py-4 space-y-2">
          ...
          <div>
            ...
            >
              <span>Hizmetler</span>
              <svg
                className={`w-4 h-4 transition-transform duration-200 ${servicesAccordionOpen ? 'rotate-180 text-sky-600' : 'text-slate-500'}`}
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                strokeWidth={2}
